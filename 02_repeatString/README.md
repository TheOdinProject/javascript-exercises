...
You will notice in this exercise that there are multiple tests (see in file `repeatString.spec.js`). Only the first test is currently enabled. So after making sure that this first one passes, enable the others one by one by deleting the `.skip` from the `test.skip()` function. If confident, you may find it easier to control f for '.skip', then click the arrow, then click replace all to test your code on all tests.
